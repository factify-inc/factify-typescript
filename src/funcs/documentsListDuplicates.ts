 ...
 */
export function documentsListDuplicates(
  client: FactifyCore,
  request: operations.ListDocumentDuplicatesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ListDocumentDuplicatesResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.ListDocumentDuplicatesRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/v1beta/documents/{document_id}/duplicates")(
    pathParams,
  );

  const query = encodeFormQuery({
    "file_contents_hash": payload.file_contents_hash,
  });
  ...
  const context = {
    ...
    operationID: "listDocumentDuplicates",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListDocumentDuplicatesResponse$inboundSchema, {
      key: "Result",
    }),
