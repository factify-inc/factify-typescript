...
import { FactifyDefaultError } from "../models/errors/factifydefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | FactifyDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | FactifyDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new FactifyDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new FactifyDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
