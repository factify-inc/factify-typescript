...
import { entryPagesGenerate } from "@factify/sdk/funcs/entryPagesGenerate.js";
...
async function run() {
  const res = await entryPagesGenerate(factify, {
    documentId: "<id>",
  });
  if (res.ok) {
    ...
    console.log("entryPagesGenerate failed:", res.error);
  }
}
